...
function FacebookPreview({ caption, mediaUrl, date, accountName }: Omit<SocialPreviewCardProps, "platform" | "optimizedVersion">) {
  return (
    <div className="rounded-lg border bg-white dark:bg-zinc-900 shadow-sm overflow-hidden font-sans text-[13px]">
      ...
      {mediaUrl ? (
        <div className="aspect-[1.91/1] w-full overflow-hidden bg-black">
      ...
      <div className="px-3 py-1.5 flex items-center justify-between text-[12px] text-muted-foreground border-b">
        <span>👍 ❤️ 😮 &nbsp;24</span>
        <span>3 comments</span>
      </div>
      <div className="flex divide-x">
        {["👍 Like", "💬 Comment", "↗ Share"].map(label => (
          <button key={label} className="flex-1 py-2 text-center text-[12px] font-medium text-muted-foreground hover:bg-muted/50 transition-colors">
            {label}
          </button>
        ))}
      </div>
...
export function SocialPreviewCard({ platform, caption, mediaUrl, date, accountName, optimizedVersion }: SocialPreviewCardProps) {
  ...
  const preview = (() => {
    switch (platform) {
      ...
      default:          return <WebsitePreview   {...props} />;
    }
  })();
