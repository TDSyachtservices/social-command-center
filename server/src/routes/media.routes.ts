...
// ─── GET /api/media/specs ────────────────────────────────────────────────────
// Returns the canonical list of platform image specs that the processor uses.
// The frontend fetches this so both sides always stay in sync — updating
// IMAGE_PLATFORM_SPECS here is the only change needed when a platform changes
// its size requirements.
router.get("/specs", (_req: Request, res: Response) => {
  sendSuccess(res, IMAGE_PLATFORM_SPECS);
});
